...
<template>
  ...
  >
    <svg
      :class="['mt-0.5 h-5 w-5 shrink-0', config.text]"
    ...
    <div class="flex-1">
      <h3
        v-if="title"
        :class="['text-sm font-semibold', config.text]"
      >
        {{ title }}
      </h3>
    ...
    <button
      ...
      :class="[
        ...
        'hover:bg-black/5 focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:outline-none',
        config.text,
      ]"
      aria-label="Dismiss"
      @click="$emit('dismiss')"
